...
<div class="container" style="width: 1000px">
  ...
  $sql = "SELECT id, title, imageurl ,comment, websiteurl, websitetitle, reviewer FROM posts";
  ...
  <div class="row row-cols-1 row-cols-md-3 g-4">
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        // (3.iii) Join output cards together with .=
        $output .=

          // (3.iv) Dynamic Data into Cards using Concatenation of Variables
          // Variables: title, comment, id, image, alt (with title), websiteurl, websitetitle,reviewer
          '
            <div class="col">
              <div class="card" id="' . $row['id'] . '">
                ...
                <div class="card-body">
                  ...
                  <p class="card-text">' . $row['reviewer'] . '</p>
                  <a href="' . $row['websiteurl'] . '" class="btn btn-primary w-100">' . $row['websitetitle'] . '</a>';


        // (3.v) Restrict Edit/Delete Button ONLY to logged in users (can see by default!)// (3.vi) Setup Edit/ Delete Pages
