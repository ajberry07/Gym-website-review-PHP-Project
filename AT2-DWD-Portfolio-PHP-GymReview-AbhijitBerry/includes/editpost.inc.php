<?php
  ...
  if(isset($_POST['edit-submit']) && isset($_SESSION['userId'])){
    ...
    if (empty($id) || empty($title) || empty($imageURL) || empty($comment) || empty($websiteURL) || empty($websiteTitle)|| empty($reviewer)) {
    ...
    // 14. Save (BY UPDATE) Edited Post to DB using Prepared Statements
      ...
      // (i) Declare Template SQL with ? Placeholders to update values for row in posts table (7 PLACEHOLDERS!)
      $sql = "UPDATE posts SET title=?, imageurl=?, comment=?, websiteurl=?, websitetitle=?, reviewer=? WHERE id=?"; 
      ...
      if(!mysqli_stmt_prepare($statement, $sql))
      {
        ...
        mysqli_stmt_bind_param($statement, "ssssssi", $title, $imageURL, $comment, $websiteURL, $websiteTitle,$reviewer, $id);
