...
if (isset($_POST['post-submit']) && isset($_SESSION['userId'])) {
  ...
  if (empty($title) || empty($imageURL) || empty($comment) || empty($websiteURL) || empty($websiteTitle) || empty($reviewer)) {
    ...
    if (!mysqli_stmt_prepare($statement, $sql)) {
      ...
      // NOTE: We bind six strings!
      mysqli_stmt_bind_param($statement, "ssssss", $title, $imageURL, $comment, $websiteURL, $websiteTitle, $reviewer);
